...
export default function LoginForm() {
    ...
    // The states below handles the authentication steps logic
    ...
    return (
        <div className={`lg:h-full h-screen w-full ${theme === 'light' ? 'bg-white' : 'bg-[#181a20]'} flex flex-col justify-between md:justify-normal`}>
            <Modal modal={modal} setModal={setModal} displayModal={displayModal} setDisplayModal={setDisplayModal} count={count} setCount={setCount} setAppAuthButton={setAppAuthButton} setEmailAuthButton={setEmailAuthButton} />
            ...
            <div className='h-full w-full flex md:justify-center flex-col md:items-center' style={{ padding }}>
                <div className={`md:border  ${theme === 'light' ? 'md:border-[#eaecef]' : 'md:border-[#2b3139]'} rounded-[24px] md:w-[425px] w-full min-h-[fit] md:min-h-[574px] md:px-[40px] md:pt-[40px] md:pb-[40px]`}>
                    ...
                    <div className='flex flex-col w-full items-center space-y-3 sm:space-y-4'>
                        ...
                        <div
                            className={`h-[72px] md:h-[64px] w-full border px-[1rem] flex cursor-pointer gap-[.75rem] rounded-[.75rem] items-center ${theme === 'dark' ? 'hover:border-white' : 'hover:border-black'
                                } ${theme === 'dark' ? 'border-[#474D57]' : 'border-gray-300'
                                } ${AppAuthButton ? 'border-none hover:border-none' : ''
                                }`}
                        ...
                        >
                            ...
                            {AppAuthButton ? (
                                ...
                                <svg fill="PrimaryText" className="bn-svg w-5 h-5 text-[--color-PrimaryText]" data-e2e="btn-mfa-step-undo" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div
                            className={`h-[72px] md:h-[64px] w-full border px-[1rem] flex cursor-pointer gap-[.75rem] rounded-[.75rem] items-center ${theme === 'dark' ? 'hover:border-white' : 'hover:border-black'
                                ...
                                } ${EmailAuthButton ? 'border-none hover:border-none' : ''
                                }`}
                            onClick={() => !EmailAuthButton && handleDisplayModal('Email')}
                            disabled={EmailAuthButton}
                        >
                            {/* Front Icon */}

                            <svg fill="PrimaryText" className="bn-svg w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path fillRule="evenodd" clipRule="evenodd" d="M21.5 5h-18v14.4h18V5zm-18 2.7l9 5.728 9-5.728v3.2l-9 5.728-9-5.727V7.7z" fill="currentColor"></path>
                            </svg>

                            <div className='flex-1 font-medium text-[16px]'>Email</div>
                            {/* Back Icon (unchanged) */}
                            {EmailAuthButton ? (
                                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#4CAF50">
                                    <path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z" />
