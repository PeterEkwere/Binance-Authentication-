            ...
            <div className='h-full w-full flex md:justify-center flex-col md:items-center' style={{ padding }}>
                <div className={`md:border ${theme === 'light' ? 'md:border-[#eaecef]' : 'md:border-[#2b3139]'} rounded-[24px] md:w-[425px] w-full min-h-[fit] md:min-h-[574px] md:px-[40px] md:pt-[40px] md:pb-[40px]`}>
                    ...
                    <div className='flex h-fit flex-col mb-1 md:mb-3 md:mt-[25px]'>
                        ...
                        <p className={`mt-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-800'} md:mt-4 text-[14px] md:text-[16px] font-normal`}>
                    ...
                    <div className='flex flex-col w-full items-center space-y-3 sm:space-y-4'>
                        ...
                        <div
                            className={`h-[72px] md:h-[64px] w-full border px-[1rem] flex cursor-pointer gap-[.75rem] rounded-[.75rem] items-center ${theme === 'dark' ? 'hover:border-white' : 'hover:border-black'} ${theme === 'dark' ? 'border-[#474D57]' : 'border-gray-300'} ${AppAuthButton ? 'border-none hover:border-none' : ''}`}
                            onClick={() => !AppAuthButton && handleDisplayModal('AuthApp')}
                            disabled={AppAuthButton}
                        >
                            <svg fill="PrimaryText" className="bn-svg w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path fillRule="evenodd" clipRule="evenodd" d="M18.5 9.5a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0zm-6.532-3.518a3.5 3.5 0 00-2.485 5.965l.01.01 2.475-2.475v-3.5zM5 18h6v3H5v-3zm8 0h6v3h-6v-3z" fill="currentColor"></path>
                            </svg>
                            <div className='flex-1 font-medium text-[16px]'>Authenticator App</div>
                            {AppAuthButton ? (
                                ...
                                <svg fill="PrimaryText" className="bn-svg w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path fillRule="evenodd" clipRule="evenodd" d="M21 11.999l-7.071-7.071-1.768 1.768 4.055 4.054H2.999v2.5h13.216l-4.054 4.053 1.768 1.768L21 12v-.001z" fill="currentColor"></path>
                                </svg>
                            )}
                        </div>
                        ...
                        {/* Phone Number Button */}
                        <div
                            className={`h-[72px] md:h-[64px] w-full border px-[1rem] flex cursor-pointer gap-[.75rem] rounded-[.75rem] items-center ${theme === 'dark' ? 'hover:border-white' : 'hover:border-black'} ${theme === 'dark' ? 'border-[#474D57]' : 'border-gray-300'} ${PhoneAuthButton ? 'border-none hover:border-none' : ''}`}
                            onClick={() => !PhoneAuthButton && handleDisplayModal('Phone')}
                            disabled={PhoneAuthButton}
                        >
                            <svg fill="currentColor" className="bn-svg w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path fillRule="evenodd" clipRule="evenodd" d="M5 2h14a2 2 0 012 2v16a2 2 0 01-2 2H5a2 2 0 01-2-2V4a2 2 0 012-2zm2 1a1 1 0 100 2 1 1 0 000-2zm10 0a1 1 0 100 2 1 1 0 000-2zm-5 16a5 5 0 100-10 5 5 0 000 10z"></path>
                            </svg>
                            <div className='flex-1 font-medium text-[16px]'>Phone Number</div>
                            {PhoneAuthButton ? (
                                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#4CAF50">
                                    <path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z" />
                                </svg>
